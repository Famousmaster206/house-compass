...
import { PreferenceWizard, type HomeSearchAnswers } from "@/components/home-search/PreferenceWizard";
...
export function HomeSearchExperience() {
  ...
  const [answers, setAnswers] = useState<HomeSearchAnswers | null>(null);
  ...
  return (
    <AnimatePresence mode="wait" initial={false}>
      ...
      {stage === "preferences" && <motion.div key="preferences" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}><PreferenceWizard onBack={() => setStage("landing")} onSubmit={(a) => { setAnswers(a); setStage("loading"); }} /></motion.div>}
      ...
      {stage === "summary" && <motion.div key="summary" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}><PropertySummary answers={answers} onRestart={() => setStage("landing")} /></motion.div>}
    </AnimatePresence>
  );
}
